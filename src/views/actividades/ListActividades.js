...
const Columnas = [
  ...
  },
  {
    id: 'Editable',
    name: 'Editable',
    selector: (row) => row.Editable,
    cell: (row) => (
      <CBadge color={row.Editable === 'Si' ? 'success' : 'danger'} shape="rounded-pill">
        {row.Editable}
      </CBadge>
    ),
    sortable: true,
    ...
    cell: (row) => (
      <>
        <CButton
          ...
          title={row.Estado === 'Activa' ? 'Eliminar actividad' : 'Restaurar actividad'}
        >
          <CIcon
        ...
        <CButton
          variant="outline"
          onClick={() => visualizar('/actividades/vista-registro', row.Codigo)}
          className="btn btn-sm btn-outline-primary ms-2"
          title={'Visualizar actividad'}
        >
...
const Formulario = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      Axios.post(URL_API_DATOS_LISTADO, formik.initialValues, CONFIG_HEADER_AUTH).then((data) => {
        if (data.data) {
  ...
  const sendData = async (dataForm) => {
    Axios.post(URL_API_DATOS_LISTADO, dataForm, CONFIG_HEADER_AUTH).then((data) => {
      ...
      {loading ? (
        ...
        <>
          ...
          <TablaReporteExportar
            ...
            botonReporte={true}
            expandible={true}
            tituloTabla={'listado actividades'}
